...
import static jdk.internal.vm.compiler.libgraal.jni.processor.AbstractFromLibGraalProcessor.topDeclaringType;
import static jdk.internal.vm.compiler.libgraal.jni.processor.AbstractFromLibGraalProcessor.createSourceFile;
 ...
 */
@SupportedAnnotationTypes("jdk.internal.vm.compiler.libgraal.jni.annotation.FromLibGraalEntryPointsResolver")
public final class FromLibGraalEntryPointsResolverProcessor extends AbstractProcessor {
    ...
    private void processElement(Element annotatedElement, DeclaredType annotationType) throws ProcessingException {
        ...
        if (method) {
            ...
            TypeMirror jclass = getTypeElement("jdk.internal.vm.compiler.libgraal.jni.JNI.JClass").asType();
        ...
        try (PrintWriter out = createSourceFile(pkg, simpleName, processingEnv.getFiler(), annotatedElement)) {
            ...
            out.println("");
            if (!pkg.equals("jdk.internal.vm.compiler.libgraal.jni")) {
                out.println("import jdk.internal.vm.compiler.libgraal.jni.FromLibGraalCalls;");
            }
            out.println("import jdk.internal.vm.compiler.libgraal.jni.JNI.JClass;");
            out.println("import jdk.internal.vm.compiler.libgraal.jni.JNI.JNIEnv;");
    ...
    private boolean verifyParameters(ExecutableElement method) {
        ...
        if (params.size() == 1) {
            ...
            TypeMirror expectedType = processingEnv.getElementUtils().getTypeElement("jdk.internal.vm.compiler.libgraal.jni.JNI.JNIEnv").asType();
