...
public abstract class AbstractFromLibGraalProcessor<T extends Enum<T> & FromLibGraalId> extends AbstractProcessor {
    ...
    private void createGenSource(CallsInfo info, String pkg, Name topDeclaringClass, Element[] originatingElements) {
        ...
        try (PrintWriter out = createSourceFile(pkg, genClassName, filer, originatingElements)) {
            ...
            out.println("");
            out.println("import jdk.internal.vm.compiler.nativeimage.StackValue;");
            out.println("import jdk.internal.vm.compiler.libgraal.jni.JNI.JNIEnv;");
            out.println("import jdk.internal.vm.compiler.libgraal.jni.JNI.JValue;");
            if (usesJObject) {
                out.println("import jdk.internal.vm.compiler.libgraal.jni.JNI.JObject;");
            }
            out.println("");
