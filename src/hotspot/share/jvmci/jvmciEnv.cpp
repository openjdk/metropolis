...
int JVMCIEnv::call_HotSpotJVMCIRuntime_adjustCompilationLevel (JVMCIObject runtime, InstanceKlass* declaringClass,
  ...
  if (is_hotspot()) {
    ...
    JavaCallArguments jargs;
    jargs.push_oop(Handle(THREAD, HotSpotJVMCI::resolve(runtime)));
    jargs.push_oop(Handle(THREAD, declaringClass->java_mirror()));
    jargs.push_oop(Handle(THREAD, HotSpotJVMCI::resolve(name)));
    jargs.push_oop(Handle(THREAD, HotSpotJVMCI::resolve(signature)));
