...
m4_define(jvm_features_valid, m4_normalize( \
    ...
    jvmci jvmti libgraal link-time-opt management minimal nmt opt-size parallelgc \
...
m4_define(jvm_feature_desc_libgraal, [include natively compiled Graal (libgraal)])
...
###############################################################################
# Check if the feature 'libgraal' is available on this platform.
#
AC_DEFUN_ONCE([JVM_FEATURES_CHECK_LIBGRAAL],
[
  ...
  UTIL_FIXUP_PATH([METROPOLIS_KIT])
  AC_SUBST(METROPOLIS_KIT)

  JVM_FEATURES_CHECK_AVAILABILITY(libgraal, [
    AC_MSG_CHECKING([if platform is supported by libgraal])
    if test "x$OPENJDK_TARGET_CPU" = "xx86_64" && \
    ...
    AC_MSG_CHECKING([if libgraal is present])
    if test -e "${METROPOLIS_KIT}/libjvmcicompiler.so"; then
      AC_MSG_RESULT([yes])
    else
      AC_MSG_RESULT([no, missing ${METROPOLIS_KIT}/libjvmcicompiler.so])
      AVAILABLE=false
    fi
  ])
])
...
[
  ...
  if test "x$variant" = "xminimal"; then
    ...
    JVM_FEATURES_VARIANT_UNAVAILABLE="aot cds compiler1 compiler2 \
        epsilongc g1gc graal jvmci libgraal minimal shenandoahgc zgc"
  else
  ...
  if test "x$variant" = "xclient"; then
    JVM_FEATURES_VARIANT_FILTER="aot compiler2 graal jvmci libgraal link-time-opt opt-size"
  elif test "x$variant" = "xminimal"; then
    JVM_FEATURES_VARIANT_FILTER="aot cds compiler2 dtrace epsilongc g1gc \
        graal jfr jni-check jvmci jvmti libgraal management nmt parallelgc services \
    ...
    JVM_FEATURES_VARIANT_FILTER="aot compiler1 compiler2 graal jvmci \
        libgraal link-time-opt opt-size"
  elif test "x$variant" = "xzero"; then
...
[
  ...
  if JVM_FEATURES_IS_ACTIVE(libgraal) && ! JVM_FEATURES_IS_ACTIVE(graal); then
    AC_MSG_ERROR([Specified JVM feature 'libgraal' requires feature 'graal' for variant '$variant'])
  fi
  ...
  if ! JVM_FEATURES_IS_ACTIVE(libgraal); then
